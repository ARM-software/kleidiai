...
/// @param[in] lhs_row_stride The number of bytes in in each row of the LHS matrix (not packed)
///
...
/// @param[in]  lhs_row_stride  Stride in bytes between two rows of LHS.
...
void kai_run_lhs_pack_f8dxp_f32_neon(
    size_t m, size_t k, size_t mr, size_t kr, size_t sr, size_t m_idx_start, const float* lhs, size_t lhs_row_stride,
    void* lhs_packed, enum kai_f8_mode mode);
#ifdef __cplusplus
}
#endif
