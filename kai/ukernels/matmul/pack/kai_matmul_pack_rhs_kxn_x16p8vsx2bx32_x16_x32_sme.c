...
/// Packs a KxN RHS matrix using the assembly micro-kernel.
///
/// @param[in] args Packing arguments.
void kai_kernel_matmul_pack_rhs_kxn_x16p8vsx2bx32_x16_x32_sme(const struct uker_args_t* args);
...
static void run(
    ...
    KAI_ASSERT(get_nr() <= MAX_NR);
    static const uint16_t pad_row[MAX_NR] = {0};
    ...
    kai_kernel_matmul_pack_rhs_kxn_x16p8vsx2bx32_x16_x32_sme(&uker_args);
}
