...
enum {
    ...
    RHS_NUM_BYTES_SUM = sizeof(float),
...
// Mirrors the untouched .S ABI of kai_kernel_matmul_clamp_f32_qai8dxp1vlx4_qsi4c32p4vlx4_1vlx4vl_sme_mopa.
typedef struct {
    float* dst;              // 0   (0x00)
...
static void run(const struct kai_matmul_uker_config* config, const struct kai_matmul_uker_args* run_args) {
    ...
    args.k = kai_roundup(run_args->shape.k, K_MULTIPLE_OF);
    args.bl = bl;
