...
/// Runs the RHS packing function for the depthwise convolution micro-kernel.
///
/// NOTE: filter_height/filter_width is separate from height/width of weights intending to allow for padding when using
///       weights shapes different to kernel conv filter size (currently unsupported). These should be the same in
///       typical usecases.
///
/// @param[in]  filter_height The height of filter being used in convolution.
/// @param[in]  filter_width  The width of filter being used in convolution.
