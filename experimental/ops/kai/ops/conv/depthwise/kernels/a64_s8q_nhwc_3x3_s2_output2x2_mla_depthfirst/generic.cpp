//
// SPDX-FileCopyrightText: Copyright 2021-2026 Arm Limited and/or its affiliates <open-source-office@arm.com>
//
// SPDX-License-Identifier: Apache-2.0
//

// clang-format off


#include "kai/ops/gemm/kai_ops.hpp"

#include <cstddef>
#include <cstdint>

#if defined(__aarch64__)

namespace kai {
namespace ops {
namespace depthwise {

void a64_s8q_nhwc_3x3_s2_output2x2_mla_depthfirst_impl(
  const unsigned int n_channels,
  const int8_t *const *const inptrs,
  const int8_t *const weights,
  const int32_t *const bias,
  const kai::ops::Requantize32 &qp,
  const int32_t *const requant_muls,
  const int32_t *const requant_shifts,
  int8_t *const *const outptrs
)
{
  struct Params
  {
    uint64_t n_channels;
    const void *weights;
    const int32_t *bias;
    const kai::ops::Requantize32 *requant;
    const int32_t *const requant_muls;
    const int32_t *const requant_shifts;
    int8_t *const *const outptrs;
    const int8_t *inptrs[25];

    Params(
      uint64_t n_channels,
      const int8_t *const *inptrs_raw,
      const void *const weights,
      const int32_t *const bias,
      const kai::ops::Requantize32 &qp,
      const int32_t *const requant_muls,
      const int32_t *const requant_shifts,
      int8_t *const *outptrs
    ) : n_channels(n_channels), weights(weights), bias(bias),
        requant(&qp), requant_muls(requant_muls),
        requant_shifts(requant_shifts), outptrs(outptrs)
    {
      inptrs[0] = inptrs_raw[12];
      inptrs[1] = inptrs_raw[0];
      inptrs[2] = inptrs_raw[1];
      inptrs[3] = inptrs_raw[3];
      inptrs[4] = inptrs_raw[4];
      inptrs[5] = inptrs_raw[5];
      inptrs[6] = inptrs_raw[6];
      inptrs[7] = inptrs_raw[2];
      inptrs[8] = inptrs_raw[8];
      inptrs[9] = inptrs_raw[9];
      inptrs[10] = inptrs_raw[7];
      inptrs[11] = inptrs_raw[15];
      inptrs[12] = inptrs_raw[10];
      inptrs[13] = inptrs_raw[16];
      inptrs[14] = inptrs_raw[11];
      inptrs[15] = inptrs_raw[18];
      inptrs[16] = inptrs_raw[13];
      inptrs[17] = inptrs_raw[19];
      inptrs[18] = inptrs_raw[20];
      inptrs[19] = inptrs_raw[14];
      inptrs[20] = inptrs_raw[21];
      inptrs[21] = inptrs_raw[17];
      inptrs[22] = inptrs_raw[23];
      inptrs[23] = inptrs_raw[22];
      inptrs[24] = inptrs_raw[24];

    }
  };

  const Params params(n_channels, inptrs, weights, bias, qp,
                      requant_muls, requant_shifts, outptrs);

  __asm__ __volatile__(
    "ldr x2, [%x[params], %[offsetof_Params_n_channels]]\n"
    "ldr x23, [%x[params], %[offsetof_Params_requant]]\n"
    "mov x3, #0\n"
    "mov x4, #0\n"
    "ldr x22, [%x[params], %[offsetof_Params_outptrs]]\n"
    "add x5, %x[params], %[offsetof_Params_inptrs]\n"
    "ldr x6, [%x[params], %[offsetof_Params_weights]]\n"
    "ldr x7, [%x[params], %[offsetof_Params_requant_muls]]\n"
    "ldr x8, [%x[params], %[offsetof_Params_requant_shifts]]\n"
    "lsr x17, x2, #0x3\n"
    "add x20, x23, %[offsetof_Requantize32_a_offset]\n"
    "ld1r { v15.16b }, [x20]\n"
    "add x21, x23, %[offsetof_Requantize32_b_offset]\n"
    "add x20, x23, %[offsetof_Requantize32_c_offset]\n"
    "ld1r { v19.16b }, [x21]\n"
    "ld1r { v11.8h }, [x20]\n"
    "add x21, x23, %[offsetof_Requantize32_minval]\n"
    "add x20, x23, %[offsetof_Requantize32_maxval]\n"
    "ld1r { v23.8h }, [x21]\n"
    "ld1r { v20.8h }, [x20]\n"
    "ldp x16, x15, [x22, #0]\n"
    "ldp x14, x13, [x22, #0x10]\n"
    "cbz x17, 3f\n"
    "ldr d8, [x6, #0]\n"
    "ldr d27, [x6, #0x8]\n"
    "subs x17, x17, #0x1\n"
    "ldr d14, [x6, #0x10]\n"
    "ldr d24, [x6, #0x18]\n"
    "ldr d16, [x6, #0x20]\n"
    "ldr d1, [x6, #0x28]\n"
    "ldr d10, [x6, #0x30]\n"
    "ldr d12, [x6, #0x38]\n"
    "ssubl v8.8h, v8.8b, v19.8b\n"
    "ssubl v27.8h, v27.8b, v19.8b\n"
    "ldr d7, [x6, #0x40]\n"
    "ldr x20, [%x[params], %[offsetof_Params_bias]]\n"
    "ssubl v14.8h, v14.8b, v19.8b\n"
    "ssubl v24.8h, v24.8b, v19.8b\n"
    "ssubl v16.8h, v16.8b, v19.8b\n"
    "ssubl v1.8h, v1.8b, v19.8b\n"
    "ssubl v10.8h, v10.8b, v19.8b\n"
    "ssubl v12.8h, v12.8b, v19.8b\n"
    "ldr q30, [x20, #0]\n"
    "ldr q31, [x20, #0x10]\n"
    "ldp x27, x26, [x5, #0]\n"
    "add x20, x20, #0x20\n"
    "ssubl v7.8h, v7.8b, v19.8b\n"
    "str x20, [%x[params], %[offsetof_Params_bias]]\n"
    "mov v18.16b, v30.16b\n"
    "mov v17.16b, v31.16b\n"
    "mov v5.16b, v30.16b\n"
    "mov v4.16b, v31.16b\n"
    "ldp x25, x24, [x5, #0x10]\n"
    "mov v2.16b, v30.16b\n"
    "mov v13.16b, v31.16b\n"
    "ldp x23, x22, [x5, #0x20]\n"
    "ldp x21, x20, [x5, #0x30]\n"
    "ldr d0, [x27, x3]\n"
    "ldr d21, [x26, x3]\n"
    "ldr d29, [x25, x3]\n"
    "ldr d6, [x24, x3]\n"
    "ldr d25, [x23, x3]\n"
    "ldr d3, [x22, x3]\n"
    "ldr d22, [x21, x3]\n"
    "ldr d9, [x20, x3]\n"
    "ssubl v0.8h, v0.8b, v15.8b\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "ssubl v29.8h, v29.8b, v15.8b\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ssubl v25.8h, v25.8b, v15.8b\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ssubl v22.8h, v22.8b, v15.8b\n"
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "beq 2f\n"
    "1:"  // Loop
    "ldr q26, [x7, #0]\n"
    "ldr q28, [x8, #0]\n"
    "smlal v30.4s, v0.4h, v7.4h\n"
    "smlal v18.4s, v0.4h, v10.4h\n"
    "ldr x24, [x5, #0x58]\n"
    "ldr x23, [x5, #0x78]\n"
    "smlal v5.4s, v0.4h, v14.4h\n"
    "smlal v2.4s, v0.4h, v8.4h\n"
    "ldr x12, [x5, #0x60]\n"
    "ldr x22, [x5, #0x80]\n"
    "smlal2 v31.4s, v0.8h, v7.8h\n"
    "smlal2 v17.4s, v0.8h, v10.8h\n"
    "smlal2 v4.4s, v0.8h, v14.8h\n"
    "smlal2 v13.4s, v0.8h, v8.8h\n"
    "ldr q0, [x7, #0x10]\n"
    "ldr x21, [x5, #0x68]\n"
    "ldr x20, [x5, #0x88]\n"
    "smlal v30.4s, v21.4h, v8.4h\n"
    "smlal v18.4s, v6.4h, v27.4h\n"
    "ldr x11, [x5, #0x40]\n"
    "ldr x10, [x5, #0x70]\n"
    "ldr x9, [x5, #0x98]\n"
    "add x6, x6, #0x48\n"
    "subs x17, x17, #0x1\n"
    "smlal2 v31.4s, v21.8h, v8.8h\n"
    "ldr d21, [x24, x3]\n"
    "smlal2 v17.4s, v6.8h, v27.8h\n"
    "ldr d6, [x23, x3]\n"
    "ldr x28, [x5, #0x48]\n"
    "ldr x27, [x5, #0x90]\n"
    "add x7, x7, #0x20\n"
    "smlal v30.4s, v29.4h, v27.4h\n"
    "smlal v18.4s, v25.4h, v14.4h\n"
    "ldr x26, [x5, #0xa8]\n"
    "ldr x25, [x5, #0x50]\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ldr x24, [x5, #0xa0]\n"
    "ldr x23, [x5, #0xb0]\n"
    "smlal2 v31.4s, v29.8h, v27.8h\n"
    "ldr d29, [x12, x3]\n"
    "smlal2 v17.4s, v25.8h, v14.8h\n"
    "ldr d25, [x22, x3]\n"
    "smlal v5.4s, v21.4h, v24.4h\n"
    "smlal2 v4.4s, v21.8h, v24.8h\n"
    "ldr d21, [x21, x3]\n"
    "smlal v30.4s, v3.4h, v24.4h\n"
    "ldr x22, [x5, #0xb8]\n"
    "smlal v2.4s, v6.4h, v16.4h\n"
    "smlal2 v13.4s, v6.8h, v16.8h\n"
    "ldr d6, [x20, x3]\n"
    "ldr x21, [x5, #0xc0]\n"
    "ssubl v29.8h, v29.8b, v15.8b\n"
    "ssubl v25.8h, v25.8b, v15.8b\n"
    "smlal v18.4s, v9.4h, v8.4h\n"
    "ldr x20, [%x[params], %[offsetof_Params_bias]]\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "smlal2 v31.4s, v3.8h, v24.8h\n"
    "ldr d3, [x11, x3]\n"
    "smlal2 v17.4s, v9.8h, v8.8h\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "smlal v30.4s, v22.4h, v16.4h\n"
    "smlal v5.4s, v29.4h, v8.4h\n"
    "smlal v2.4s, v25.4h, v27.4h\n"
    "smlal2 v4.4s, v29.8h, v8.8h\n"
    "ldr d8, [x10, x3]\n"
    "smlal2 v13.4s, v25.8h, v27.8h\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "smlal2 v31.4s, v22.8h, v16.8h\n"
    "ldr d22, [x9, x3]\n"
    "smlal v5.4s, v21.4h, v16.4h\n"
    "ssubl v8.8h, v8.8b, v15.8b\n"
    "smlal v30.4s, v9.4h, v14.4h\n"
    "smlal v2.4s, v6.4h, v1.4h\n"
    "ssubl v22.8h, v22.8b, v15.8b\n"
    "smlal2 v4.4s, v21.8h, v16.8h\n"
    "ldr d21, [x28, x3]\n"
    "smlal v18.4s, v3.4h, v16.4h\n"
    "smlal2 v17.4s, v3.8h, v16.8h\n"
    "ldr d16, [x27, x3]\n"
    "ldr d3, [x26, x3]\n"
    "smlal2 v13.4s, v6.8h, v1.8h\n"
    "ldr d6, [x25, x3]\n"
    "smlal2 v31.4s, v9.8h, v14.8h\n"
    "ldr d9, [x24, x3]\n"
    "smlal v5.4s, v8.4h, v27.4h\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "smlal v2.4s, v22.4h, v14.4h\n"
    "ssubl v16.8h, v16.8b, v15.8b\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "smlal2 v4.4s, v8.8h, v27.8h\n"
    "ldr d27, [x23, x3]\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "smlal2 v13.4s, v22.8h, v14.8h\n"
    "ldr d14, [x22, x3]\n"
    "smlal v18.4s, v21.4h, v1.4h\n"
    "smlal2 v17.4s, v21.8h, v1.8h\n"
    "ldr d21, [x21, x3]\n"
    "add x3, x3, #0x8\n"
    "smlal v5.4s, v16.4h, v10.4h\n"
    "smlal v2.4s, v3.4h, v24.4h\n"
    "ssubl v27.8h, v27.8b, v15.8b\n"
    "smlal2 v4.4s, v16.8h, v10.8h\n"
    "ldr q16, [x8, #0x10]\n"
    "smlal v30.4s, v6.4h, v1.4h\n"
    "ssubl v14.8h, v14.8b, v15.8b\n"
    "smlal2 v13.4s, v3.8h, v24.8h\n"
    "smlal2 v31.4s, v6.8h, v1.8h\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "add x8, x8, #0x20\n"
    "smlal v18.4s, v6.4h, v24.4h\n"
    "smlal2 v17.4s, v6.8h, v24.8h\n"
    "smlal v5.4s, v9.4h, v12.4h\n"
    "smlal v2.4s, v27.4h, v12.4h\n"
    "smlal2 v4.4s, v9.8h, v12.8h\n"
    "smlal v30.4s, v29.4h, v10.4h\n"
    "smlal2 v13.4s, v27.8h, v12.8h\n"
    "smlal2 v31.4s, v29.8h, v10.8h\n"
    "smlal v18.4s, v25.4h, v12.4h\n"
    "smlal2 v17.4s, v25.8h, v12.8h\n"
    "smlal v5.4s, v3.4h, v1.4h\n"
    "smlal v2.4s, v14.4h, v10.4h\n"
    "smlal2 v4.4s, v3.8h, v1.8h\n"
    "smlal v30.4s, v8.4h, v12.4h\n"
    "smlal2 v13.4s, v14.8h, v10.8h\n"
    "smlal2 v31.4s, v8.8h, v12.8h\n"
    "smlal v18.4s, v22.4h, v7.4h\n"
    "smlal2 v17.4s, v22.8h, v7.8h\n"
    "smlal v5.4s, v14.4h, v7.4h\n"
    "smlal v2.4s, v21.4h, v7.4h\n"
    "smlal2 v4.4s, v14.8h, v7.8h\n"
    "sqdmulh v30.4s, v30.4s, v26.4s\n"
    "smlal2 v13.4s, v21.8h, v7.8h\n"
    "sqdmulh v31.4s, v31.4s, v0.4s\n"
    "sqdmulh v18.4s, v18.4s, v26.4s\n"
    "sqdmulh v17.4s, v17.4s, v0.4s\n"
    "sqdmulh v5.4s, v5.4s, v26.4s\n"
    "sqdmulh v2.4s, v2.4s, v26.4s\n"
    "srshl v30.4s, v30.4s, v28.4s\n"
    "srshl v18.4s, v18.4s, v28.4s\n"
    "sqdmulh v4.4s, v4.4s, v0.4s\n"
    "srshl v5.4s, v5.4s, v28.4s\n"
    "sqdmulh v13.4s, v13.4s, v0.4s\n"
    "srshl v2.4s, v2.4s, v28.4s\n"
    "srshl v31.4s, v31.4s, v16.4s\n"
    "sqxtn v30.4h, v30.4s\n"
    "srshl v17.4s, v17.4s, v16.4s\n"
    "sqxtn v18.4h, v18.4s\n"
    "srshl v4.4s, v4.4s, v16.4s\n"
    "sqxtn v5.4h, v5.4s\n"
    "srshl v13.4s, v13.4s, v16.4s\n"
    "sqxtn v2.4h, v2.4s\n"
    "sqxtn2 v30.8h, v31.4s\n"
    "sqxtn2 v18.8h, v17.4s\n"
    "sqxtn2 v5.8h, v4.4s\n"
    "sqxtn2 v2.8h, v13.4s\n"
    "sqadd v30.8h, v30.8h, v11.8h\n"
    "sqadd v18.8h, v18.8h, v11.8h\n"
    "sqadd v5.8h, v5.8h, v11.8h\n"
    "sqadd v2.8h, v2.8h, v11.8h\n"
    "smax v30.8h, v30.8h, v23.8h\n"
    "smax v18.8h, v18.8h, v23.8h\n"
    "smax v5.8h, v5.8h, v23.8h\n"
    "smax v2.8h, v2.8h, v23.8h\n"
    "smin v30.8h, v30.8h, v20.8h\n"
    "smin v18.8h, v18.8h, v20.8h\n"
    "smin v5.8h, v5.8h, v20.8h\n"
    "smin v2.8h, v2.8h, v20.8h\n"
    "uzp1 v30.16b, v30.16b, v30.16b\n"
    "uzp1 v18.16b, v18.16b, v18.16b\n"
    "uzp1 v5.16b, v5.16b, v5.16b\n"
    "uzp1 v2.16b, v2.16b, v2.16b\n"
    "str d30, [x16, x4]\n"
    "str d18, [x15, x4]\n"
    "str d5, [x14, x4]\n"
    "str d2, [x13, x4]\n"
    "add x4, x4, #0x8\n"
    "ldr q30, [x20, #0]\n"
    "ldr q31, [x20, #0x10]\n"
    "add x20, x20, #0x20\n"
    "str x20, [%x[params], %[offsetof_Params_bias]]\n"
    "ldr d8, [x6, #0]\n"
    "ldr d27, [x6, #0x8]\n"
    "ldr d14, [x6, #0x10]\n"
    "ldr d24, [x6, #0x18]\n"
    "ldr d16, [x6, #0x20]\n"
    "ldr d1, [x6, #0x28]\n"
    "mov v18.16b, v30.16b\n"
    "mov v17.16b, v31.16b\n"
    "ldr d10, [x6, #0x30]\n"
    "ldr d12, [x6, #0x38]\n"
    "mov v5.16b, v30.16b\n"
    "mov v4.16b, v31.16b\n"
    "ldr d7, [x6, #0x40]\n"
    "ldp x27, x26, [x5, #0]\n"
    "mov v2.16b, v30.16b\n"
    "mov v13.16b, v31.16b\n"
    "ssubl v8.8h, v8.8b, v19.8b\n"
    "ssubl v27.8h, v27.8b, v19.8b\n"
    "ssubl v14.8h, v14.8b, v19.8b\n"
    "ssubl v24.8h, v24.8b, v19.8b\n"
    "ldp x25, x24, [x5, #0x10]\n"
    "ssubl v16.8h, v16.8b, v19.8b\n"
    "ssubl v1.8h, v1.8b, v19.8b\n"
    "ssubl v10.8h, v10.8b, v19.8b\n"
    "ssubl v12.8h, v12.8b, v19.8b\n"
    "ldp x23, x22, [x5, #0x20]\n"
    "ssubl v7.8h, v7.8b, v19.8b\n"
    "ldp x21, x20, [x5, #0x30]\n"
    "ldr d0, [x27, x3]\n"
    "ldr d21, [x26, x3]\n"
    "ldr d29, [x25, x3]\n"
    "ldr d6, [x24, x3]\n"
    "ldr d25, [x23, x3]\n"
    "ldr d3, [x22, x3]\n"
    "ldr d22, [x21, x3]\n"
    "ssubl v0.8h, v0.8b, v15.8b\n"
    "ldr d9, [x20, x3]\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "ssubl v29.8h, v29.8b, v15.8b\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ssubl v25.8h, v25.8b, v15.8b\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ssubl v22.8h, v22.8b, v15.8b\n"
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "bgt 1b\n"
    "2:"  // Tail
    "ldr q28, [x7, #0]\n"
    "ldr q26, [x8, #0]\n"
    "smlal v30.4s, v0.4h, v7.4h\n"
    "smlal v18.4s, v0.4h, v10.4h\n"
    "ldr x23, [x5, #0x58]\n"
    "ldr x22, [x5, #0x78]\n"
    "smlal v5.4s, v0.4h, v14.4h\n"
    "smlal v2.4s, v0.4h, v8.4h\n"
    "ldr x12, [x5, #0x60]\n"
    "ldr x11, [x5, #0x80]\n"
    "smlal2 v31.4s, v0.8h, v7.8h\n"
    "smlal2 v17.4s, v0.8h, v10.8h\n"
    "smlal2 v4.4s, v0.8h, v14.8h\n"
    "smlal2 v13.4s, v0.8h, v8.8h\n"
    "ldr q0, [x7, #0x10]\n"
    "ldr x21, [x5, #0x68]\n"
    "ldr x20, [x5, #0x88]\n"
    "smlal v30.4s, v21.4h, v8.4h\n"
    "smlal v18.4s, v6.4h, v27.4h\n"
    "ldr x10, [x5, #0x40]\n"
    "ldr x9, [x5, #0x70]\n"
    "ldr x28, [x5, #0x98]\n"
    "tst x2, #0x7\n"
    "add x7, x7, #0x20\n"
    "smlal2 v31.4s, v21.8h, v8.8h\n"
    "ldr d21, [x23, x3]\n"
    "smlal2 v17.4s, v6.8h, v27.8h\n"
    "ldr d6, [x22, x3]\n"
    "ldr x27, [x5, #0x48]\n"
    "ldr x26, [x5, #0x90]\n"
    "smlal v30.4s, v29.4h, v27.4h\n"
    "smlal v18.4s, v25.4h, v14.4h\n"
    "ldr x25, [x5, #0xa8]\n"
    "ldr x24, [x5, #0x50]\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ldr x23, [x5, #0xa0]\n"
    "ldr x22, [x5, #0xb0]\n"
    "smlal2 v31.4s, v29.8h, v27.8h\n"
    "ldr d29, [x12, x3]\n"
    "smlal2 v17.4s, v25.8h, v14.8h\n"
    "ldr d25, [x11, x3]\n"
    "smlal v5.4s, v21.4h, v24.4h\n"
    "smlal v2.4s, v6.4h, v16.4h\n"
    "smlal2 v4.4s, v21.8h, v24.8h\n"
    "ldr d21, [x21, x3]\n"
    "ldr x21, [x5, #0xb8]\n"
    "smlal2 v13.4s, v6.8h, v16.8h\n"
    "ldr d6, [x20, x3]\n"
    "smlal v30.4s, v3.4h, v24.4h\n"
    "ldr x20, [x5, #0xc0]\n"
    "ssubl v29.8h, v29.8b, v15.8b\n"
    "ssubl v25.8h, v25.8b, v15.8b\n"
    "smlal v18.4s, v9.4h, v8.4h\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "smlal2 v31.4s, v3.8h, v24.8h\n"
    "ldr d3, [x10, x3]\n"
    "smlal2 v17.4s, v9.8h, v8.8h\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "smlal v5.4s, v29.4h, v8.4h\n"
    "smlal v2.4s, v25.4h, v27.4h\n"
    "smlal2 v4.4s, v29.8h, v8.8h\n"
    "ldr d8, [x9, x3]\n"
    "smlal2 v13.4s, v25.8h, v27.8h\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "smlal v30.4s, v22.4h, v16.4h\n"
    "smlal2 v31.4s, v22.8h, v16.8h\n"
    "ldr d22, [x28, x3]\n"
    "smlal v5.4s, v21.4h, v16.4h\n"
    "smlal v2.4s, v6.4h, v1.4h\n"
    "ssubl v8.8h, v8.8b, v15.8b\n"
    "smlal v18.4s, v3.4h, v16.4h\n"
    "smlal2 v17.4s, v3.8h, v16.8h\n"
    "ldr d3, [x27, x3]\n"
    "ssubl v22.8h, v22.8b, v15.8b\n"
    "smlal2 v4.4s, v21.8h, v16.8h\n"
    "ldr d16, [x26, x3]\n"
    "ldr d21, [x25, x3]\n"
    "smlal2 v13.4s, v6.8h, v1.8h\n"
    "ldr d6, [x24, x3]\n"
    "smlal v30.4s, v9.4h, v14.4h\n"
    "smlal2 v31.4s, v9.8h, v14.8h\n"
    "ldr d9, [x23, x3]\n"
    "smlal v5.4s, v8.4h, v27.4h\n"
    "smlal v2.4s, v22.4h, v14.4h\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ssubl v16.8h, v16.8b, v15.8b\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "smlal2 v4.4s, v8.8h, v27.8h\n"
    "ldr d27, [x22, x3]\n"
    "smlal2 v13.4s, v22.8h, v14.8h\n"
    "ldr d14, [x21, x3]\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "smlal v18.4s, v3.4h, v1.4h\n"
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "smlal2 v17.4s, v3.8h, v1.8h\n"
    "ldr d3, [x20, x3]\n"
    "add x3, x3, #0x8\n"
    "smlal v5.4s, v16.4h, v10.4h\n"
    "smlal v2.4s, v21.4h, v24.4h\n"
    "ssubl v27.8h, v27.8b, v15.8b\n"
    "smlal2 v4.4s, v16.8h, v10.8h\n"
    "ldr q16, [x8, #0x10]\n"
    "smlal v30.4s, v6.4h, v1.4h\n"
    "ssubl v14.8h, v14.8b, v15.8b\n"
    "smlal2 v13.4s, v21.8h, v24.8h\n"
    "smlal v18.4s, v6.4h, v24.4h\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "add x8, x8, #0x20\n"
    "smlal2 v31.4s, v6.8h, v1.8h\n"
    "smlal2 v17.4s, v6.8h, v24.8h\n"
    "smlal v5.4s, v9.4h, v12.4h\n"
    "smlal v2.4s, v27.4h, v12.4h\n"
    "smlal2 v4.4s, v9.8h, v12.8h\n"
    "smlal v30.4s, v29.4h, v10.4h\n"
    "smlal2 v13.4s, v27.8h, v12.8h\n"
    "smlal v18.4s, v25.4h, v12.4h\n"
    "smlal2 v31.4s, v29.8h, v10.8h\n"
    "smlal2 v17.4s, v25.8h, v12.8h\n"
    "smlal v5.4s, v21.4h, v1.4h\n"
    "smlal v2.4s, v14.4h, v10.4h\n"
    "smlal2 v4.4s, v21.8h, v1.8h\n"
    "smlal v30.4s, v8.4h, v12.4h\n"
    "smlal2 v13.4s, v14.8h, v10.8h\n"
    "smlal v18.4s, v22.4h, v7.4h\n"
    "smlal2 v31.4s, v8.8h, v12.8h\n"
    "smlal2 v17.4s, v22.8h, v7.8h\n"
    "smlal v5.4s, v14.4h, v7.4h\n"
    "smlal v2.4s, v3.4h, v7.4h\n"
    "smlal2 v4.4s, v14.8h, v7.8h\n"
    "sqdmulh v30.4s, v30.4s, v28.4s\n"
    "smlal2 v13.4s, v3.8h, v7.8h\n"
    "sqdmulh v18.4s, v18.4s, v28.4s\n"
    "sqdmulh v31.4s, v31.4s, v0.4s\n"
    "sqdmulh v17.4s, v17.4s, v0.4s\n"
    "sqdmulh v5.4s, v5.4s, v28.4s\n"
    "sqdmulh v2.4s, v2.4s, v28.4s\n"
    "srshl v30.4s, v30.4s, v26.4s\n"
    "srshl v18.4s, v18.4s, v26.4s\n"
    "sqdmulh v4.4s, v4.4s, v0.4s\n"
    "srshl v5.4s, v5.4s, v26.4s\n"
    "sqdmulh v13.4s, v13.4s, v0.4s\n"
    "srshl v2.4s, v2.4s, v26.4s\n"
    "srshl v31.4s, v31.4s, v16.4s\n"
    "sqxtn v30.4h, v30.4s\n"
    "srshl v17.4s, v17.4s, v16.4s\n"
    "sqxtn v18.4h, v18.4s\n"
    "srshl v4.4s, v4.4s, v16.4s\n"
    "sqxtn v5.4h, v5.4s\n"
    "srshl v13.4s, v13.4s, v16.4s\n"
    "sqxtn v2.4h, v2.4s\n"
    "sqxtn2 v30.8h, v31.4s\n"
    "sqxtn2 v18.8h, v17.4s\n"
    "sqxtn2 v5.8h, v4.4s\n"
    "sqxtn2 v2.8h, v13.4s\n"
    "sqadd v30.8h, v30.8h, v11.8h\n"
    "sqadd v18.8h, v18.8h, v11.8h\n"
    "sqadd v5.8h, v5.8h, v11.8h\n"
    "sqadd v2.8h, v2.8h, v11.8h\n"
    "smax v30.8h, v30.8h, v23.8h\n"
    "smax v18.8h, v18.8h, v23.8h\n"
    "smax v5.8h, v5.8h, v23.8h\n"
    "smax v2.8h, v2.8h, v23.8h\n"
    "smin v30.8h, v30.8h, v20.8h\n"
    "smin v18.8h, v18.8h, v20.8h\n"
    "smin v5.8h, v5.8h, v20.8h\n"
    "smin v2.8h, v2.8h, v20.8h\n"
    "uzp1 v30.16b, v30.16b, v30.16b\n"
    "uzp1 v18.16b, v18.16b, v18.16b\n"
    "uzp1 v5.16b, v5.16b, v5.16b\n"
    "uzp1 v2.16b, v2.16b, v2.16b\n"
    "str d30, [x16, x4]\n"
    "str d18, [x15, x4]\n"
    "str d5, [x14, x4]\n"
    "str d2, [x13, x4]\n"
    "add x4, x4, #0x8\n"
    "beq 88f\n"
    "add x6, x6, #0x48\n"
    "3:"  // Oddments
    "ldr x20, [%x[params], %[offsetof_Params_bias]]\n"
    "tbz x2, #2, 5f\n"
    "ld1 { v30.4s }, [x20], #0x10\n"
    "tbz x2, #1, 4f\n"
    "ld1 { v31.d }[0], [x20], #0x8\n"
    "tbz x2, #0, 7f\n"
    "ld1 { v31.s }[2], [x20]\n"
    "b 7f\n"
    "4:"  // Oddments: Load bias: Bit 2: Bit 1: Unset
    "tbz x2, #0, 7f\n"
    "ld1 { v31.s }[0], [x20]\n"
    "b 7f\n"
    "5:"  // Oddments: Load bias: Bit 2: Unset
    "tbz x2, #1, 6f\n"
    "ld1 { v30.d }[0], [x20], #0x8\n"
    "tbz x2, #0, 7f\n"
    "ld1 { v30.s }[2], [x20]\n"
    "b 7f\n"
    "6:"  // Oddments: Load bias: Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 7f\n"
    "ld1 { v30.s }[0], [x20]\n"
    "7:"  // Oddments: Load bias: Bit 2: End
    "ldr d8, [x6, #0]\n"
    "ldr d27, [x6, #0x8]\n"
    "mov v18.16b, v30.16b\n"
    "mov v17.16b, v31.16b\n"
    "ldr d14, [x6, #0x10]\n"
    "ldr d24, [x6, #0x18]\n"
    "mov v5.16b, v30.16b\n"
    "mov v4.16b, v31.16b\n"
    "ldr d16, [x6, #0x20]\n"
    "ldr d1, [x6, #0x28]\n"
    "mov v2.16b, v30.16b\n"
    "mov v13.16b, v31.16b\n"
    "ldr d10, [x6, #0x30]\n"
    "ldr d12, [x6, #0x38]\n"
    "ssubl v8.8h, v8.8b, v19.8b\n"
    "ssubl v27.8h, v27.8b, v19.8b\n"
    "ldr d7, [x6, #0x40]\n"
    "ldp x27, x26, [x5, #0]\n"
    "ssubl v14.8h, v14.8b, v19.8b\n"
    "ssubl v24.8h, v24.8b, v19.8b\n"
    "ssubl v16.8h, v16.8b, v19.8b\n"
    "ssubl v1.8h, v1.8b, v19.8b\n"
    "ssubl v10.8h, v10.8b, v19.8b\n"
    "ssubl v12.8h, v12.8b, v19.8b\n"
    "ldp x25, x24, [x5, #0x10]\n"
    "ssubl v7.8h, v7.8b, v19.8b\n"
    "add x27, x27, x3\n"
    "add x26, x26, x3\n"
    "ldp x23, x22, [x5, #0x20]\n"
    "add x25, x25, x3\n"
    "add x24, x24, x3\n"
    "ldp x21, x20, [x5, #0x30]\n"
    "add x23, x23, x3\n"
    "add x22, x22, x3\n"
    "add x21, x21, x3\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 9f\n"
    "ld1 { v0.s }[0], [x27], #0x4\n"
    "ld1 { v21.s }[0], [x26], #0x4\n"
    "ld1 { v29.s }[0], [x25], #0x4\n"
    "ld1 { v6.s }[0], [x24], #0x4\n"
    "ld1 { v25.s }[0], [x23], #0x4\n"
    "ld1 { v3.s }[0], [x22], #0x4\n"
    "ld1 { v22.s }[0], [x21], #0x4\n"
    "ld1 { v9.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 8f\n"
    "ld1 { v0.h }[2], [x27], #0x2\n"
    "ld1 { v21.h }[2], [x26], #0x2\n"
    "ld1 { v29.h }[2], [x25], #0x2\n"
    "ld1 { v6.h }[2], [x24], #0x2\n"
    "ld1 { v25.h }[2], [x23], #0x2\n"
    "ld1 { v3.h }[2], [x22], #0x2\n"
    "ld1 { v22.h }[2], [x21], #0x2\n"
    "ld1 { v9.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 11f\n"
    "ld1 { v0.b }[6], [x27]\n"
    "ld1 { v21.b }[6], [x26]\n"
    "ld1 { v29.b }[6], [x25]\n"
    "ld1 { v6.b }[6], [x24]\n"
    "ld1 { v25.b }[6], [x23]\n"
    "ld1 { v3.b }[6], [x22]\n"
    "ld1 { v22.b }[6], [x21]\n"
    "ld1 { v9.b }[6], [x20]\n"
    "b 11f\n"
    "8:"  // Oddments: Initial loads: Bit 2: Bit 1: Unset
    "tbz x2, #0, 11f\n"
    "ld1 { v0.b }[4], [x27]\n"
    "ld1 { v21.b }[4], [x26]\n"
    "ld1 { v29.b }[4], [x25]\n"
    "ld1 { v6.b }[4], [x24]\n"
    "ld1 { v25.b }[4], [x23]\n"
    "ld1 { v3.b }[4], [x22]\n"
    "ld1 { v22.b }[4], [x21]\n"
    "ld1 { v9.b }[4], [x20]\n"
    "b 11f\n"
    "9:"  // Oddments: Initial loads: Bit 2: Unset
    "tbz x2, #1, 10f\n"
    "ld1 { v0.h }[0], [x27], #0x2\n"
    "ld1 { v21.h }[0], [x26], #0x2\n"
    "ld1 { v29.h }[0], [x25], #0x2\n"
    "ld1 { v6.h }[0], [x24], #0x2\n"
    "ld1 { v25.h }[0], [x23], #0x2\n"
    "ld1 { v3.h }[0], [x22], #0x2\n"
    "ld1 { v22.h }[0], [x21], #0x2\n"
    "ld1 { v9.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 11f\n"
    "ld1 { v0.b }[2], [x27]\n"
    "ld1 { v21.b }[2], [x26]\n"
    "ld1 { v29.b }[2], [x25]\n"
    "ld1 { v6.b }[2], [x24]\n"
    "ld1 { v25.b }[2], [x23]\n"
    "ld1 { v3.b }[2], [x22]\n"
    "ld1 { v22.b }[2], [x21]\n"
    "ld1 { v9.b }[2], [x20]\n"
    "b 11f\n"
    "10:"  // Oddments: Initial loads: Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 11f\n"
    "ld1 { v0.b }[0], [x27]\n"
    "ld1 { v21.b }[0], [x26]\n"
    "ld1 { v29.b }[0], [x25]\n"
    "ld1 { v6.b }[0], [x24]\n"
    "ld1 { v25.b }[0], [x23]\n"
    "ld1 { v3.b }[0], [x22]\n"
    "ld1 { v22.b }[0], [x21]\n"
    "ld1 { v9.b }[0], [x20]\n"
    "11:"  // Oddments: Initial loads: Bit 2: End
    "ssubl v0.8h, v0.8b, v15.8b\n"
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "ldr x20, [x5, #0x40]\n"
    "ssubl v29.8h, v29.8b, v15.8b\n"
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ssubl v25.8h, v25.8b, v15.8b\n"
    "smlal v30.4s, v0.4h, v7.4h\n"
    "smlal2 v31.4s, v0.8h, v7.8h\n"
    "smlal v18.4s, v0.4h, v10.4h\n"
    "ssubl v22.8h, v22.8b, v15.8b\n"
    "add x20, x20, x3\n"
    "smlal2 v17.4s, v0.8h, v10.8h\n"
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "smlal v5.4s, v0.4h, v14.4h\n"
    "smlal2 v4.4s, v0.8h, v14.8h\n"
    "smlal v2.4s, v0.4h, v8.4h\n"
    "smlal v30.4s, v21.4h, v8.4h\n"
    "smlal2 v13.4s, v0.8h, v8.8h\n"
    "smlal2 v31.4s, v21.8h, v8.8h\n"
    "smlal v18.4s, v6.4h, v27.4h\n"
    "smlal2 v17.4s, v6.8h, v27.8h\n"
    "smlal v30.4s, v29.4h, v27.4h\n"
    "smlal2 v31.4s, v29.8h, v27.8h\n"
    "smlal v18.4s, v25.4h, v14.4h\n"
    "smlal v30.4s, v3.4h, v24.4h\n"
    "smlal2 v17.4s, v25.8h, v14.8h\n"
    "smlal2 v31.4s, v3.8h, v24.8h\n"
    "smlal v18.4s, v9.4h, v8.4h\n"
    "smlal v30.4s, v22.4h, v16.4h\n"
    "smlal2 v17.4s, v9.8h, v8.8h\n"
    "smlal2 v31.4s, v22.8h, v16.8h\n"
    "smlal v30.4s, v9.4h, v14.4h\n"
    "smlal2 v31.4s, v9.8h, v14.8h\n"
    "tbz x2, #2, 13f\n"
    "ld1 { v19.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 12f\n"
    "ld1 { v19.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 15f\n"
    "ld1 { v19.b }[6], [x20]\n"
    "b 15f\n"
    "12:"  // Oddments: Load (1, 3): Bit 2: Bit 1: Unset
    "tbz x2, #0, 15f\n"
    "ld1 { v19.b }[4], [x20]\n"
    "b 15f\n"
    "13:"  // Oddments: Load (1, 3): Bit 2: Unset
    "tbz x2, #1, 14f\n"
    "ld1 { v19.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 15f\n"
    "ld1 { v19.b }[2], [x20]\n"
    "b 15f\n"
    "14:"  // Oddments: Load (1, 3): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 15f\n"
    "ld1 { v19.b }[0], [x20]\n"
    "15:"  // Oddments: Load (1, 3): Bit 2: End
    "ssubl v19.8h, v19.8b, v15.8b\n"
    "ldr x20, [x5, #0x48]\n"
    "smlal v18.4s, v19.4h, v16.4h\n"
    "smlal2 v17.4s, v19.8h, v16.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 17f\n"
    "ld1 { v9.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 16f\n"
    "ld1 { v9.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 19f\n"
    "ld1 { v9.b }[6], [x20]\n"
    "b 19f\n"
    "16:"  // Oddments: Load (1, 4): Bit 2: Bit 1: Unset
    "tbz x2, #0, 19f\n"
    "ld1 { v9.b }[4], [x20]\n"
    "b 19f\n"
    "17:"  // Oddments: Load (1, 4): Bit 2: Unset
    "tbz x2, #1, 18f\n"
    "ld1 { v9.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 19f\n"
    "ld1 { v9.b }[2], [x20]\n"
    "b 19f\n"
    "18:"  // Oddments: Load (1, 4): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 19f\n"
    "ld1 { v9.b }[0], [x20]\n"
    "19:"  // Oddments: Load (1, 4): Bit 2: End
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "ldr x20, [x5, #0x50]\n"
    "smlal v18.4s, v9.4h, v1.4h\n"
    "smlal2 v17.4s, v9.8h, v1.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 21f\n"
    "ld1 { v26.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 20f\n"
    "ld1 { v26.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 23f\n"
    "ld1 { v26.b }[6], [x20]\n"
    "b 23f\n"
    "20:"  // Oddments: Load (1, 2): Bit 2: Bit 1: Unset
    "tbz x2, #0, 23f\n"
    "ld1 { v26.b }[4], [x20]\n"
    "b 23f\n"
    "21:"  // Oddments: Load (1, 2): Bit 2: Unset
    "tbz x2, #1, 22f\n"
    "ld1 { v26.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 23f\n"
    "ld1 { v26.b }[2], [x20]\n"
    "b 23f\n"
    "22:"  // Oddments: Load (1, 2): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 23f\n"
    "ld1 { v26.b }[0], [x20]\n"
    "23:"  // Oddments: Load (1, 2): Bit 2: End
    "ssubl v26.8h, v26.8b, v15.8b\n"
    "ldr x20, [x5, #0x58]\n"
    "smlal v30.4s, v26.4h, v1.4h\n"
    "smlal2 v31.4s, v26.8h, v1.8h\n"
    "smlal v18.4s, v26.4h, v24.4h\n"
    "smlal2 v17.4s, v26.8h, v24.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 25f\n"
    "ld1 { v0.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 24f\n"
    "ld1 { v0.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 27f\n"
    "ld1 { v0.b }[6], [x20]\n"
    "b 27f\n"
    "24:"  // Oddments: Load (3, 0): Bit 2: Bit 1: Unset
    "tbz x2, #0, 27f\n"
    "ld1 { v0.b }[4], [x20]\n"
    "b 27f\n"
    "25:"  // Oddments: Load (3, 0): Bit 2: Unset
    "tbz x2, #1, 26f\n"
    "ld1 { v0.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 27f\n"
    "ld1 { v0.b }[2], [x20]\n"
    "b 27f\n"
    "26:"  // Oddments: Load (3, 0): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 27f\n"
    "ld1 { v0.b }[0], [x20]\n"
    "27:"  // Oddments: Load (3, 0): Bit 2: End
    "ssubl v0.8h, v0.8b, v15.8b\n"
    "ldr x20, [x5, #0x60]\n"
    "smlal v5.4s, v0.4h, v24.4h\n"
    "smlal2 v4.4s, v0.8h, v24.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 29f\n"
    "ld1 { v19.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 28f\n"
    "ld1 { v19.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 31f\n"
    "ld1 { v19.b }[6], [x20]\n"
    "b 31f\n"
    "28:"  // Oddments: Load (2, 0): Bit 2: Bit 1: Unset
    "tbz x2, #0, 31f\n"
    "ld1 { v19.b }[4], [x20]\n"
    "b 31f\n"
    "29:"  // Oddments: Load (2, 0): Bit 2: Unset
    "tbz x2, #1, 30f\n"
    "ld1 { v19.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 31f\n"
    "ld1 { v19.b }[2], [x20]\n"
    "b 31f\n"
    "30:"  // Oddments: Load (2, 0): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 31f\n"
    "ld1 { v19.b }[0], [x20]\n"
    "31:"  // Oddments: Load (2, 0): Bit 2: End
    "ssubl v19.8h, v19.8b, v15.8b\n"
    "ldr x20, [x5, #0x68]\n"
    "smlal v30.4s, v19.4h, v10.4h\n"
    "smlal2 v31.4s, v19.8h, v10.8h\n"
    "smlal v5.4s, v19.4h, v8.4h\n"
    "smlal2 v4.4s, v19.8h, v8.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 33f\n"
    "ld1 { v26.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 32f\n"
    "ld1 { v26.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 35f\n"
    "ld1 { v26.b }[6], [x20]\n"
    "b 35f\n"
    "32:"  // Oddments: Load (3, 1): Bit 2: Bit 1: Unset
    "tbz x2, #0, 35f\n"
    "ld1 { v26.b }[4], [x20]\n"
    "b 35f\n"
    "33:"  // Oddments: Load (3, 1): Bit 2: Unset
    "tbz x2, #1, 34f\n"
    "ld1 { v26.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 35f\n"
    "ld1 { v26.b }[2], [x20]\n"
    "b 35f\n"
    "34:"  // Oddments: Load (3, 1): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 35f\n"
    "ld1 { v26.b }[0], [x20]\n"
    "35:"  // Oddments: Load (3, 1): Bit 2: End
    "ssubl v26.8h, v26.8b, v15.8b\n"
    "ldr x20, [x5, #0x70]\n"
    "smlal v5.4s, v26.4h, v16.4h\n"
    "smlal2 v4.4s, v26.8h, v16.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 37f\n"
    "ld1 { v3.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 36f\n"
    "ld1 { v3.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 39f\n"
    "ld1 { v3.b }[6], [x20]\n"
    "b 39f\n"
    "36:"  // Oddments: Load (2, 1): Bit 2: Bit 1: Unset
    "tbz x2, #0, 39f\n"
    "ld1 { v3.b }[4], [x20]\n"
    "b 39f\n"
    "37:"  // Oddments: Load (2, 1): Bit 2: Unset
    "tbz x2, #1, 38f\n"
    "ld1 { v3.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 39f\n"
    "ld1 { v3.b }[2], [x20]\n"
    "b 39f\n"
    "38:"  // Oddments: Load (2, 1): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 39f\n"
    "ld1 { v3.b }[0], [x20]\n"
    "39:"  // Oddments: Load (2, 1): Bit 2: End
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ldr x20, [x5, #0x78]\n"
    "smlal v30.4s, v3.4h, v12.4h\n"
    "smlal2 v31.4s, v3.8h, v12.8h\n"
    "smlal v5.4s, v3.4h, v27.4h\n"
    "smlal2 v4.4s, v3.8h, v27.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 41f\n"
    "ld1 { v26.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 40f\n"
    "ld1 { v26.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 43f\n"
    "ld1 { v26.b }[6], [x20]\n"
    "b 43f\n"
    "40:"  // Oddments: Load (3, 3): Bit 2: Bit 1: Unset
    "tbz x2, #0, 43f\n"
    "ld1 { v26.b }[4], [x20]\n"
    "b 43f\n"
    "41:"  // Oddments: Load (3, 3): Bit 2: Unset
    "tbz x2, #1, 42f\n"
    "ld1 { v26.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 43f\n"
    "ld1 { v26.b }[2], [x20]\n"
    "b 43f\n"
    "42:"  // Oddments: Load (3, 3): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 43f\n"
    "ld1 { v26.b }[0], [x20]\n"
    "43:"  // Oddments: Load (3, 3): Bit 2: End
    "ssubl v26.8h, v26.8b, v15.8b\n"
    "ldr x20, [x5, #0x80]\n"
    "smlal v2.4s, v26.4h, v16.4h\n"
    "smlal2 v13.4s, v26.8h, v16.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 45f\n"
    "ld1 { v21.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 44f\n"
    "ld1 { v21.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 47f\n"
    "ld1 { v21.b }[6], [x20]\n"
    "b 47f\n"
    "44:"  // Oddments: Load (2, 3): Bit 2: Bit 1: Unset
    "tbz x2, #0, 47f\n"
    "ld1 { v21.b }[4], [x20]\n"
    "b 47f\n"
    "45:"  // Oddments: Load (2, 3): Bit 2: Unset
    "tbz x2, #1, 46f\n"
    "ld1 { v21.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 47f\n"
    "ld1 { v21.b }[2], [x20]\n"
    "b 47f\n"
    "46:"  // Oddments: Load (2, 3): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 47f\n"
    "ld1 { v21.b }[0], [x20]\n"
    "47:"  // Oddments: Load (2, 3): Bit 2: End
    "ssubl v21.8h, v21.8b, v15.8b\n"
    "ldr x20, [x5, #0x88]\n"
    "smlal v18.4s, v21.4h, v12.4h\n"
    "smlal2 v17.4s, v21.8h, v12.8h\n"
    "smlal v2.4s, v21.4h, v27.4h\n"
    "smlal2 v13.4s, v21.8h, v27.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 49f\n"
    "ld1 { v9.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 48f\n"
    "ld1 { v9.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 51f\n"
    "ld1 { v9.b }[6], [x20]\n"
    "b 51f\n"
    "48:"  // Oddments: Load (3, 4): Bit 2: Bit 1: Unset
    "tbz x2, #0, 51f\n"
    "ld1 { v9.b }[4], [x20]\n"
    "b 51f\n"
    "49:"  // Oddments: Load (3, 4): Bit 2: Unset
    "tbz x2, #1, 50f\n"
    "ld1 { v9.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 51f\n"
    "ld1 { v9.b }[2], [x20]\n"
    "b 51f\n"
    "50:"  // Oddments: Load (3, 4): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 51f\n"
    "ld1 { v9.b }[0], [x20]\n"
    "51:"  // Oddments: Load (3, 4): Bit 2: End
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "ldr x20, [x5, #0x90]\n"
    "smlal v2.4s, v9.4h, v1.4h\n"
    "smlal2 v13.4s, v9.8h, v1.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 53f\n"
    "ld1 { v0.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 52f\n"
    "ld1 { v0.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 55f\n"
    "ld1 { v0.b }[6], [x20]\n"
    "b 55f\n"
    "52:"  // Oddments: Load (4, 0): Bit 2: Bit 1: Unset
    "tbz x2, #0, 55f\n"
    "ld1 { v0.b }[4], [x20]\n"
    "b 55f\n"
    "53:"  // Oddments: Load (4, 0): Bit 2: Unset
    "tbz x2, #1, 54f\n"
    "ld1 { v0.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 55f\n"
    "ld1 { v0.b }[2], [x20]\n"
    "b 55f\n"
    "54:"  // Oddments: Load (4, 0): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 55f\n"
    "ld1 { v0.b }[0], [x20]\n"
    "55:"  // Oddments: Load (4, 0): Bit 2: End
    "ssubl v0.8h, v0.8b, v15.8b\n"
    "ldr x20, [x5, #0x98]\n"
    "smlal v5.4s, v0.4h, v10.4h\n"
    "smlal2 v4.4s, v0.8h, v10.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 57f\n"
    "ld1 { v3.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 56f\n"
    "ld1 { v3.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 59f\n"
    "ld1 { v3.b }[6], [x20]\n"
    "b 59f\n"
    "56:"  // Oddments: Load (2, 4): Bit 2: Bit 1: Unset
    "tbz x2, #0, 59f\n"
    "ld1 { v3.b }[4], [x20]\n"
    "b 59f\n"
    "57:"  // Oddments: Load (2, 4): Bit 2: Unset
    "tbz x2, #1, 58f\n"
    "ld1 { v3.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 59f\n"
    "ld1 { v3.b }[2], [x20]\n"
    "b 59f\n"
    "58:"  // Oddments: Load (2, 4): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 59f\n"
    "ld1 { v3.b }[0], [x20]\n"
    "59:"  // Oddments: Load (2, 4): Bit 2: End
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ldr x20, [x5, #0xa0]\n"
    "smlal v18.4s, v3.4h, v7.4h\n"
    "smlal2 v17.4s, v3.8h, v7.8h\n"
    "smlal v2.4s, v3.4h, v14.4h\n"
    "smlal2 v13.4s, v3.8h, v14.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 61f\n"
    "ld1 { v9.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 60f\n"
    "ld1 { v9.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 63f\n"
    "ld1 { v9.b }[6], [x20]\n"
    "b 63f\n"
    "60:"  // Oddments: Load (4, 1): Bit 2: Bit 1: Unset
    "tbz x2, #0, 63f\n"
    "ld1 { v9.b }[4], [x20]\n"
    "b 63f\n"
    "61:"  // Oddments: Load (4, 1): Bit 2: Unset
    "tbz x2, #1, 62f\n"
    "ld1 { v9.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 63f\n"
    "ld1 { v9.b }[2], [x20]\n"
    "b 63f\n"
    "62:"  // Oddments: Load (4, 1): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 63f\n"
    "ld1 { v9.b }[0], [x20]\n"
    "63:"  // Oddments: Load (4, 1): Bit 2: End
    "ssubl v9.8h, v9.8b, v15.8b\n"
    "ldr x20, [x5, #0xa8]\n"
    "smlal v5.4s, v9.4h, v12.4h\n"
    "smlal2 v4.4s, v9.8h, v12.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 65f\n"
    "ld1 { v6.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 64f\n"
    "ld1 { v6.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 67f\n"
    "ld1 { v6.b }[6], [x20]\n"
    "b 67f\n"
    "64:"  // Oddments: Load (3, 2): Bit 2: Bit 1: Unset
    "tbz x2, #0, 67f\n"
    "ld1 { v6.b }[4], [x20]\n"
    "b 67f\n"
    "65:"  // Oddments: Load (3, 2): Bit 2: Unset
    "tbz x2, #1, 66f\n"
    "ld1 { v6.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 67f\n"
    "ld1 { v6.b }[2], [x20]\n"
    "b 67f\n"
    "66:"  // Oddments: Load (3, 2): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 67f\n"
    "ld1 { v6.b }[0], [x20]\n"
    "67:"  // Oddments: Load (3, 2): Bit 2: End
    "ssubl v6.8h, v6.8b, v15.8b\n"
    "ldr x20, [x5, #0xb0]\n"
    "smlal v5.4s, v6.4h, v1.4h\n"
    "smlal2 v4.4s, v6.8h, v1.8h\n"
    "smlal v2.4s, v6.4h, v24.4h\n"
    "smlal2 v13.4s, v6.8h, v24.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 69f\n"
    "ld1 { v22.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 68f\n"
    "ld1 { v22.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 71f\n"
    "ld1 { v22.b }[6], [x20]\n"
    "b 71f\n"
    "68:"  // Oddments: Load (4, 3): Bit 2: Bit 1: Unset
    "tbz x2, #0, 71f\n"
    "ld1 { v22.b }[4], [x20]\n"
    "b 71f\n"
    "69:"  // Oddments: Load (4, 3): Bit 2: Unset
    "tbz x2, #1, 70f\n"
    "ld1 { v22.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 71f\n"
    "ld1 { v22.b }[2], [x20]\n"
    "b 71f\n"
    "70:"  // Oddments: Load (4, 3): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 71f\n"
    "ld1 { v22.b }[0], [x20]\n"
    "71:"  // Oddments: Load (4, 3): Bit 2: End
    "ssubl v22.8h, v22.8b, v15.8b\n"
    "ldr x20, [x5, #0xb8]\n"
    "smlal v2.4s, v22.4h, v12.4h\n"
    "smlal2 v13.4s, v22.8h, v12.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 73f\n"
    "ld1 { v3.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 72f\n"
    "ld1 { v3.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 75f\n"
    "ld1 { v3.b }[6], [x20]\n"
    "b 75f\n"
    "72:"  // Oddments: Load (4, 2): Bit 2: Bit 1: Unset
    "tbz x2, #0, 75f\n"
    "ld1 { v3.b }[4], [x20]\n"
    "b 75f\n"
    "73:"  // Oddments: Load (4, 2): Bit 2: Unset
    "tbz x2, #1, 74f\n"
    "ld1 { v3.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 75f\n"
    "ld1 { v3.b }[2], [x20]\n"
    "b 75f\n"
    "74:"  // Oddments: Load (4, 2): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 75f\n"
    "ld1 { v3.b }[0], [x20]\n"
    "75:"  // Oddments: Load (4, 2): Bit 2: End
    "ssubl v3.8h, v3.8b, v15.8b\n"
    "ldr x20, [x5, #0xc0]\n"
    "smlal v5.4s, v3.4h, v7.4h\n"
    "smlal2 v4.4s, v3.8h, v7.8h\n"
    "smlal v2.4s, v3.4h, v10.4h\n"
    "smlal2 v13.4s, v3.8h, v10.8h\n"
    "add x20, x20, x3\n"
    "tbz x2, #2, 77f\n"
    "ld1 { v19.s }[0], [x20], #0x4\n"
    "tbz x2, #1, 76f\n"
    "ld1 { v19.h }[2], [x20], #0x2\n"
    "tbz x2, #0, 79f\n"
    "ld1 { v19.b }[6], [x20]\n"
    "b 79f\n"
    "76:"  // Oddments: Load (4, 4): Bit 2: Bit 1: Unset
    "tbz x2, #0, 79f\n"
    "ld1 { v19.b }[4], [x20]\n"
    "b 79f\n"
    "77:"  // Oddments: Load (4, 4): Bit 2: Unset
    "tbz x2, #1, 78f\n"
    "ld1 { v19.h }[0], [x20], #0x2\n"
    "tbz x2, #0, 79f\n"
    "ld1 { v19.b }[2], [x20]\n"
    "b 79f\n"
    "78:"  // Oddments: Load (4, 4): Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 79f\n"
    "ld1 { v19.b }[0], [x20]\n"
    "79:"  // Oddments: Load (4, 4): Bit 2: End
    "ssubl v19.8h, v19.8b, v15.8b\n"
    "smlal v2.4s, v19.4h, v7.4h\n"
    "smlal2 v13.4s, v19.8h, v7.8h\n"
    "tbz x2, #2, 81f\n"
    "ld1 { v16.4s }, [x7], #0x10\n"
    "ld1 { v8.4s }, [x8], #0x10\n"
    "tbz x2, #1, 80f\n"
    "ld1 { v3.d }[0], [x7], #0x8\n"
    "ld1 { v29.d }[0], [x8], #0x8\n"
    "tbz x2, #0, 83f\n"
    "ld1 { v3.s }[2], [x7]\n"
    "ld1 { v29.s }[2], [x8]\n"
    "b 83f\n"
    "80:"  // Oddments: Load requant params: Bit 2: Bit 1: Unset
    "tbz x2, #0, 83f\n"
    "ld1 { v3.s }[0], [x7]\n"
    "ld1 { v29.s }[0], [x8]\n"
    "b 83f\n"
    "81:"  // Oddments: Load requant params: Bit 2: Unset
    "tbz x2, #1, 82f\n"
    "ld1 { v16.d }[0], [x7], #0x8\n"
    "ld1 { v8.d }[0], [x8], #0x8\n"
    "tbz x2, #0, 83f\n"
    "ld1 { v16.s }[2], [x7]\n"
    "ld1 { v8.s }[2], [x8]\n"
    "b 83f\n"
    "82:"  // Oddments: Load requant params: Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 83f\n"
    "ld1 { v16.s }[0], [x7]\n"
    "ld1 { v8.s }[0], [x8]\n"
    "83:"  // Oddments: Load requant params: Bit 2: End
    "sqdmulh v30.4s, v30.4s, v16.4s\n"
    "sqdmulh v18.4s, v18.4s, v16.4s\n"
    "add x16, x16, x4\n"
    "add x15, x15, x4\n"
    "sqdmulh v5.4s, v5.4s, v16.4s\n"
    "sqdmulh v2.4s, v2.4s, v16.4s\n"
    "add x14, x14, x4\n"
    "add x13, x13, x4\n"
    "sqdmulh v31.4s, v31.4s, v3.4s\n"
    "sqdmulh v17.4s, v17.4s, v3.4s\n"
    "srshl v30.4s, v30.4s, v8.4s\n"
    "srshl v18.4s, v18.4s, v8.4s\n"
    "srshl v5.4s, v5.4s, v8.4s\n"
    "sqdmulh v4.4s, v4.4s, v3.4s\n"
    "srshl v2.4s, v2.4s, v8.4s\n"
    "sqdmulh v13.4s, v13.4s, v3.4s\n"
    "srshl v31.4s, v31.4s, v29.4s\n"
    "sqxtn v30.4h, v30.4s\n"
    "srshl v17.4s, v17.4s, v29.4s\n"
    "sqxtn v18.4h, v18.4s\n"
    "srshl v4.4s, v4.4s, v29.4s\n"
    "sqxtn v5.4h, v5.4s\n"
    "srshl v13.4s, v13.4s, v29.4s\n"
    "sqxtn v2.4h, v2.4s\n"
    "sqxtn2 v30.8h, v31.4s\n"
    "sqxtn2 v18.8h, v17.4s\n"
    "sqxtn2 v5.8h, v4.4s\n"
    "sqxtn2 v2.8h, v13.4s\n"
    "sqadd v30.8h, v30.8h, v11.8h\n"
    "sqadd v18.8h, v18.8h, v11.8h\n"
    "sqadd v5.8h, v5.8h, v11.8h\n"
    "sqadd v2.8h, v2.8h, v11.8h\n"
    "smax v30.8h, v30.8h, v23.8h\n"
    "smax v18.8h, v18.8h, v23.8h\n"
    "smax v5.8h, v5.8h, v23.8h\n"
    "smax v2.8h, v2.8h, v23.8h\n"
    "smin v30.8h, v30.8h, v20.8h\n"
    "smin v18.8h, v18.8h, v20.8h\n"
    "smin v5.8h, v5.8h, v20.8h\n"
    "smin v2.8h, v2.8h, v20.8h\n"
    "uzp1 v30.16b, v30.16b, v30.16b\n"
    "uzp1 v18.16b, v18.16b, v18.16b\n"
    "uzp1 v5.16b, v5.16b, v5.16b\n"
    "uzp1 v2.16b, v2.16b, v2.16b\n"
    "tbz x2, #2, 85f\n"
    "st1 { v30.s }[0], [x16], #0x4\n"
    "st1 { v18.s }[0], [x15], #0x4\n"
    "st1 { v5.s }[0], [x14], #0x4\n"
    "st1 { v2.s }[0], [x13], #0x4\n"
    "tbz x2, #1, 84f\n"
    "st1 { v30.h }[2], [x16], #0x2\n"
    "st1 { v18.h }[2], [x15], #0x2\n"
    "st1 { v5.h }[2], [x14], #0x2\n"
    "st1 { v2.h }[2], [x13], #0x2\n"
    "tbz x2, #0, 87f\n"
    "st1 { v30.b }[6], [x16], #0x1\n"
    "st1 { v18.b }[6], [x15], #0x1\n"
    "st1 { v5.b }[6], [x14], #0x1\n"
    "st1 { v2.b }[6], [x13], #0x1\n"
    "b 87f\n"
    "84:"  // Oddments: Bit 2: Bit 1: Unset
    "tbz x2, #0, 87f\n"
    "st1 { v30.b }[4], [x16], #0x1\n"
    "st1 { v18.b }[4], [x15], #0x1\n"
    "st1 { v5.b }[4], [x14], #0x1\n"
    "st1 { v2.b }[4], [x13], #0x1\n"
    "b 87f\n"
    "85:"  // Oddments: Bit 2: Unset
    "tbz x2, #1, 86f\n"
    "st1 { v30.h }[0], [x16], #0x2\n"
    "st1 { v18.h }[0], [x15], #0x2\n"
    "st1 { v5.h }[0], [x14], #0x2\n"
    "st1 { v2.h }[0], [x13], #0x2\n"
    "tbz x2, #0, 87f\n"
    "st1 { v30.b }[2], [x16], #0x1\n"
    "st1 { v18.b }[2], [x15], #0x1\n"
    "st1 { v5.b }[2], [x14], #0x1\n"
    "st1 { v2.b }[2], [x13], #0x1\n"
    "b 87f\n"
    "86:"  // Oddments: Bit 2: Unset: Bit 1: Unset
    "tbz x2, #0, 87f\n"
    "st1 { v30.b }[0], [x16], #0x1\n"
    "st1 { v18.b }[0], [x15], #0x1\n"
    "st1 { v5.b }[0], [x14], #0x1\n"
    "st1 { v2.b }[0], [x13], #0x1\n"
    "87:"  // Oddments: Bit 2: End
    "88:"  // End
    :
    : [offsetof_Params_bias] "I" (offsetof(Params, bias)), [offsetof_Params_inptrs] "I" (offsetof(Params, inptrs)), [offsetof_Params_n_channels] "I" (offsetof(Params, n_channels)), [offsetof_Params_outptrs] "I" (offsetof(Params, outptrs)), [offsetof_Params_requant] "I" (offsetof(Params, requant)), [offsetof_Params_requant_muls] "I" (offsetof(Params, requant_muls)), [offsetof_Params_requant_shifts] "I" (offsetof(Params, requant_shifts)), [offsetof_Params_weights] "I" (offsetof(Params, weights)), [offsetof_Requantize32_a_offset] "I" (offsetof(kai::ops::Requantize32, a_offset)), [offsetof_Requantize32_b_offset] "I" (offsetof(kai::ops::Requantize32, b_offset)), [offsetof_Requantize32_c_offset] "I" (offsetof(kai::ops::Requantize32, c_offset)), [offsetof_Requantize32_maxval] "I" (offsetof(kai::ops::Requantize32, maxval)), [offsetof_Requantize32_minval] "I" (offsetof(kai::ops::Requantize32, minval)), [params] "r" (&params)
    : "cc", "memory", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23", "v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9", "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27", "x28"
  );
}

}  // namespace depthwise
}  // namespace ops
}  // namespace kai

#endif  // defined(__aarch64__)
